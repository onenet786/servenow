...
function initializeAdmin() {
    ...
    const fuelMgmtBtn = document.getElementById('fuelManagementBtn');
    if (fuelMgmtBtn) {
        fuelMgmtBtn.addEventListener('click', () => {
            const panel = document.getElementById('riderFuelContainer');
            if (!panel) {
                showInfo('Fuel Management', 'Fuel panel not available on this page');
                return;
            }
            ...
            if (isHidden) {
                ...
                // Load riders, then load the selected rider's history (if any),
                // and only then scroll so the panel stays in view after content loads.
                loadRidersForFuelSelect()
                .then(() => {
                    const sel = document.getElementById('fuelRiderSelect');
                    if (sel && sel.value) return loadFuelHistory(sel.value);
                    // if none selected, focus the select so user can pick
                    if (sel) sel.focus();
                    return Promise.resolve();
                })
                .then(() => {
                    // center the panel in the viewport after table populates
                    panel.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    panel.style.transition = 'box-shadow 0.25s ease';
                    panel.style.boxShadow = '0 0 0 4px rgba(0,123,255,0.12)';
                    setTimeout(() => { panel.style.boxShadow = ''; }, 1200);
                })
                .catch(err => {
                    console.error('Error initializing fuel panel:', err);
                    // still bring panel into view as fallback
                    panel.scrollIntoView({ behavior: 'smooth', block: 'center' });
                });
            } else {
