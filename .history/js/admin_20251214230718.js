...
async function saveRider() {
    ...
    try {
        const uploadImage = async (file) => {
            ...
            const p1 = await tryUpload(`${API_BASE}/api/admin/upload-image`);
            if (p1) return p1;
            const p2 = await tryUpload(`${API_BASE}/api/categories/upload-image`);
            if (p2) return p2;
            const p3 = await tryUpload(`${API_BASE}/api/stores/upload-image`);
            if (p3) return p3;
            const p4 = await tryUpload(`${API_BASE}/api/products/upload-image`);
            return p4;
        };
