...
async function displayRiderDeliveries(status = 'assigned') {
    ...
    try {
        const response = await fetch(`http://192.168.55.245:3000/api/orders/rider/deliveries?status=${status}`, {
            headers: {
...
async function updateMyLocation(orderId) {
    ...
    try {
        const response = await fetch(`http://192.168.55.245:3000/api/orders/${orderId}/rider-location`, {
            method: 'PUT',
            headers: {
...
async function markDelivered(orderId) {
    ...
    try {
        const response = await fetch(`http://192.168.55.245:3000/api/orders/${orderId}/deliver`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('serveNowToken')}`
            }
...
async function updatePaymentStatus(orderId, status) {
    ...
    try {
        const response = await fetch(`http://192.168.55.245:3000/api/orders/${orderId}/payment-status`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('serveNowToken')}`
...
async function loadRiderInfo() {
    try {
        const response = await fetch('http://192.168.55.245:3000/api/orders/rider/profile', {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('serveNowToken')}`
            }
        });
