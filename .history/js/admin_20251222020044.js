...
function recalcProductPrice() {
    ...
    const typeEl = document.getElementById('productPricingAdjustmentType');
    const valEl = document.getElementById('productPricingAdjustmentValue');
    const storeEl = document.getElementById('productStore');
    if (!priceEl || !costEl || !typeEl || !valEl || !storeEl) return;
    const cost = parseFloat(String(costEl.value || '').trim());
    const adjVal = parseFloat(String(valEl.value || '').trim());
    const type = String(typeEl.value || '').toLowerCase();
    const term = productStoreTermsById[String(storeEl.value || '')] || '';
    if (!Number.isFinite(cost) || cost < 0) return;
    if (!Number.isFinite(adjVal) || adjVal < 0) return;
    const adj = type === 'percent' ? (cost * (adjVal / 100)) : adjVal;
    const hasDiscount = term.toLowerCase().includes('with discount');
    let out = hasDiscount ? (cost - adj) : (cost + adj);
    if (!Number.isFinite(out)) return;
    if (out < 0) out = 0;
    priceEl.value = (Math.round(out * 100) / 100).toFixed(2);
}
...
function bindProductPriceCalc() {
    ...
    ['productStore', 'productCostPrice', 'productPricingAdjustmentType', 'productPricingAdjustmentValue'].forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.addEventListener('change', recalcProductPrice);
        el.addEventListener('input', recalcProductPrice);
    });
}
...
async function showAddProductModal() {
    ...
    try {
        ...
        try {
            ...
            const valEl = document.getElementById('productPricingAdjustmentValue');
            const priceEl = document.getElementById('productPrice');
            if (typeEl && !typeEl.value) typeEl.value = 'percent';
...
async function saveProduct() {
    ...
    const rawAdjType = formData.get('pricing_adjustment_type');
    const rawAdjVal = formData.get('pricing_adjustment_value');
    const costPriceVal = parseFloat(String(rawCostPrice || '').trim());
    const adjVal = parseFloat(String(rawAdjVal || '').trim());
    const adjType = String(rawAdjType || '').toLowerCase();

    if (!Number.isInteger(storeId) || storeId <= 0) {
        showError('Invalid Input', 'Please select a store');
        return;
    }
    if (!Number.isFinite(costPriceVal) || costPriceVal < 0) {
        showError('Invalid Input', 'Please enter a valid cost price');
        return;
    }
    if (adjType !== 'percent' && adjType !== 'amount') {
        showError('Invalid Input', 'Please select a valid adjustment type');
        return;
    }
    if (!Number.isFinite(adjVal) || adjVal < 0) {
        showError('Invalid Input', 'Please enter a valid adjustment value');
        return;
    }
    ...
    try { recalcProductPrice(); } catch (e) {}
    const priceVal = parseFloat(String(document.getElementById('productPrice')?.value || '').trim());
    if (!Number.isFinite(priceVal) || priceVal < 0) {
        showError('Invalid Input', 'Unable to calculate a valid price');
        return;
    }
    ...
    const productData = {
        ...
        pricing_adjustment_type: adjType,
        pricing_adjustment_value: adjVal,
        category_id: formData.get('category_id') || null,
        store_id: storeId,
...
async function editProduct(productId) {
    ...
    try {
        ...
        if (form.querySelector('#productPricingAdjustmentType') && !form.querySelector('#productPricingAdjustmentType').value) form.querySelector('#productPricingAdjustmentType').value = 'percent';
        if (form.querySelector('#productPricingAdjustmentValue')) form.querySelector('#productPricingAdjustmentValue').value = '';
