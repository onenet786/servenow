...
async function saveProduct() {
    ...
    const costPriceVal = rawCost.length ? parseFloat(rawCost) : null;
    ...
    if (!isEditing && (!Number.isFinite(costPriceVal) || costPriceVal < 0)) {
        showError('Invalid Input', 'Please enter a valid cost price');
        return;
    }
    if (costPriceVal !== null && (!Number.isFinite(costPriceVal) || costPriceVal < 0)) {
        showError('Invalid Input', 'Please enter a valid cost price');
        return;
    }
    ...
    if (costPriceVal !== null) {
        productData.cost_price = costPriceVal;
    }
