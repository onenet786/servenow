...
async function addToCart(productId, productName, price, stockQty, unitName) {
...
function isFractionalUnit(name) {
    const n = String(name || '').toLowerCase();
    return n === 'kilogram' || n === 'kg' || n === 'liter' || n === 'litre' || n === 'l';
}

function qtyStepForUnit(name) {
    return isFractionalUnit(name) ? 0.25 : 1;
}

function setCartItemQuantity(productId, quantity) {
    const item = cart.find(i => i.id === productId);
    const isFrac = item ? isFractionalUnit(item.unitName) : false;
    const qRaw = isFrac ? parseFloat(quantity) : parseInt(quantity, 10);
    const base = Number.isFinite(qRaw) ? qRaw : 1;
    const minQ = qtyStepForUnit(item?.unitName);
...
async function incrementQty(productId) {
    ...
    const step = qtyStepForUnit(item?.unitName);
        ...
        itemElement.innerHTML = `
            <div class="cart-item-info">
                ...
                <div class="cart-qty">
                    ...
                    <input type="range" class="qty-slider" min="${isFractionalUnit(item.unitName) ? qtyStepForUnit(item.unitName) : 1}" max="${Number.isFinite(item.maxQty) ? item.maxQty : 20}" step="${qtyStepForUnit(item.unitName)}" value="${item.quantity}" oninput="changeQty(${item.id}, this.value)">
                    ${isFractionalUnit(item.unitName) ? `<input type="text" class="qty-value-input" value="${item.quantity}" oninput="changeQty(${item.id}, this.value)" inputmode="decimal">` : `<span class="qty-value">${item.quantity}</span>`}
            ...
            data.products.forEach(product => {
                ...
                productCard.innerHTML = `
                    ...
                    <div class="product-card-content">
                        ...
                        <button class="add-to-cart" onclick="addToCart(${product.id}, '${product.name}', ${product.price}, ${Number.isFinite(parseInt(product.stock_quantity)) ? parseInt(product.stock_quantity,10) : 'undefined'}, '${String(product.unit_name || '').replace(/'/g, "\\'")}')">Add to Cart</button>
                    </div>
                `;
