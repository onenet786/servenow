...
async function saveRider() {
    ...
    try {
        const uploadImage = async (file) => {
            ...
            // Prefer riders endpoint; fallback to categories (admin-permitted)
            const primary = await tryUpload(`${API_BASE}/api/riders/upload-image`);
            if (primary) return primary;
            const fallback = await tryUpload(`${API_BASE}/api/categories/upload-image`);
            return fallback;
        };
