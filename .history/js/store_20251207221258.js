...
function displayStoreProducts(storeId) {
    ...
    storeProducts.forEach(product => {
        ...
        if (product.image) {
            ...
            if (/^https?:\/\//i.test(url) || url.startsWith('/') || url.toLowerCase().startsWith('data:')) {
                imageSrc = url;
            } else {
        ...
        productCard.innerHTML = `
            <img src="${imageSrc}" alt="${product.name}" onerror="this.onerror=null;this.src='https://via.placeholder.com/200x150/E0E0E0/666666?text=No+Image'; console.warn('Store product image failed to load:', ${product.id}, this.src)">
