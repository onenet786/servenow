...
function displayStoreProducts(storeId) {
    ...
    storeProducts.forEach(product => {
        ...
        // Apply orientation fit for cached images in this card
        productCard.querySelectorAll('img').forEach(img => {
            try {
                if (img.complete && img.naturalWidth && img.naturalHeight) applyOrientationFitStore(img);
            } catch (e) { /* ignore */ }
        });
    });
}
...
function buildImgTagForStore(src, variants, alt, pid) {
    ...
    if (variants && typeof variants === 'object') {
        ...
        return `<img src="${smallest || src || fallback}" srcset="${entries}" sizes="(max-width: 600px) 50vw, (max-width: 1200px) 33vw, 25vw" alt="${safeAlt}" loading="lazy" decoding="async" onload="applyOrientationFitStore(this)" onerror="this.onerror=null;this.src='${fallback}'; console.warn('Store product image failed to load:', '${pid}', this.src)">`;
    }
    return `<img src="${src || fallback}" alt="${safeAlt}" loading="lazy" decoding="async" onload="applyOrientationFitStore(this)" onerror="this.onerror=null;this.src='${fallback}'; console.warn('Store product image failed to load:', '${pid}', this.src)">`;
}
