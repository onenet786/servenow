...
// Admin preview orientation helper (global so store and product modals can use it)
...
async function showAddStoreModal() {
    ...
    try {
        ...
        // Setup image URL/file preview and paste helper for store image (match product modal)
        try {
        ...
        if (fileInput && fileInput.files && fileInput.files.length > 0) {
            try {
                ...
                if (upJson.success && upJson.image_url) {
                    ...
                    console.warn('Store image upload returned no URL, using provided image_url if any');
                }
            } catch (err) {
