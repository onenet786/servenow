...
// Load products by category (include admin=true when logged-in user is admin)
async function loadProducts(category) {
    ...
    try {
        // If a user is stored in localStorage and is an admin, request admin view
        let queryParams = [];
        if (category) queryParams.push(`category=${encodeURIComponent(category)}`);

        try {
            const localUser = JSON.parse(localStorage.getItem('serveNowUser') || 'null');
            if (localUser && localUser.user_type === 'admin') {
                queryParams.push('admin=true');
            }
        } catch (e) {
            // ignore JSON parse errors and continue without admin flag
            console.warn('Failed to parse stored user for admin flag', e);
        }
