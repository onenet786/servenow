...
function collectProductSizePrices() {
    ...
    for (const row of rows) {
        const sizeEl = row.querySelector('select[data-role="size-price-size"]');
        const unitEl = row.querySelector('select[data-role="size-price-unit"]');
        const priceEl = row.querySelector('input[data-role="size-price-price"]');
        const sizeId = sizeEl ? parseInt(String(sizeEl.value || ''), 10) : NaN;
        const unitId = unitEl ? parseInt(String(unitEl.value || ''), 10) : NaN;
        const price = priceEl ? parseFloat(String(priceEl.value || '').trim()) : NaN;
        if (!Number.isInteger(sizeId) || sizeId <= 0) continue;
        if (!Number.isInteger(unitId) || unitId <= 0) continue;
        if (!Number.isFinite(price) || price < 0) continue;
        const key = `${sizeId}|${unitId}`;
        if (seen.has(key)) continue;
        seen.add(key);
        out.push({ size_id: sizeId, unit_id: unitId, price: Math.round(price * 100) / 100 });
    }
    return out;
}
...
function setProductSizePricesEnabled(enabled) {
    ...
    if (single) single.style.display = enabled ? 'none' : '';
...
function addProductSizePriceRow(prefill) {
    ...
    tr.innerHTML = `
        ...
        <td></td>
    `;
    const sizeTd = tr.children[0];
    const unitTd = tr.children[1];
    const priceTd = tr.children[2];
    const actionTd = tr.children[3];

    const sizeSelect = document.createElement('select');
    sizeSelect.setAttribute('data-role', 'size-price-size');
    sizeSelect.style.minWidth = '260px';
    sizeSelect.innerHTML = '<option value="">Select Size</option>' + (currentSizes || []).map(s => `<option value="${s.id}">${s.label}</option>`).join('');
    if (prefill && prefill.size_id) sizeSelect.value = String(prefill.size_id);

    const unitSelect = document.createElement('select');
    unitSelect.setAttribute('data-role', 'size-price-unit');
    unitSelect.style.minWidth = '200px';
    unitSelect.innerHTML = '<option value="">Select Unit</option>' + (currentUnits || []).map(u => `<option value="${u.id}">${u.name}${u.abbreviation ? ' ('+u.abbreviation+')' : ''}</option>`).join('');
    if (prefill && prefill.unit_id) unitSelect.value = String(prefill.unit_id);
    ...
    sizeSelect.addEventListener('change', syncProductPriceFromSizePrices);
    unitSelect.addEventListener('change', syncProductPriceFromSizePrices);
...
function refreshProductSizePriceRowOptions() {
    const tbody = document.getElementById('productSizePricesBody');
    if (!tbody) return;
    const selects = Array.from(tbody.querySelectorAll('select[data-role="size-price-size"]'));
    ...
    const unitSelects = Array.from(tbody.querySelectorAll('select[data-role="size-price-unit"]'));
    ...
    try {
        ...
        try {
            ...
            if (shouldUseVariants) {
                ...
                const tbody = document.getElementById('productSizePricesBody');
                if (tbody) tbody.innerHTML = '';
                (p.size_variants || []).forEach(v => addProductSizePriceRow({ size_id: v.size_id, price: v.price }));
            } else {
                setProductSizePricesEnabled(false);
            }
        } catch (e) {}
