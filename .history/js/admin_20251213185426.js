...
async function saveCategory() {
    ...
    if (catFileInput && catFileInput.files && catFileInput.files.length > 0) {
        try {
            const urlVal = (categoryData.image_url || '').trim();
            if (!urlVal) {
                delete categoryData.image_url;
                try { showWarning('Image Upload', 'File upload not supported. Paste an image URL instead.'); } catch (e) {}
            }
        } catch (e) {}
    }
