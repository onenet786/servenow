...
async function editUnit(unitId) {
    ...
    if (u) {
        // Ensure modal exists; do not reset the form (avoid showAddUnitModal which resets)
        let modal = document.getElementById('addUnitModal');
        if (!modal) {
            const m = document.createElement('div');
            m.id = 'addUnitModal';
        ...
        // Show the modal first, then populate
        showModal('addUnitModal');
        const form = document.getElementById('addUnitForm') || (modal && modal.querySelector('form'));
        if (form) {
            const nameInput = (modal && modal.querySelector('#unitName')) || form.querySelector('#unitName') || document.getElementById('unitName');
            const abbrInput = (modal && modal.querySelector('#unitAbbrev')) || form.querySelector('#unitAbbrev') || document.getElementById('unitAbbrev');
            const multInput = (modal && modal.querySelector('#unitMultiplier')) || form.querySelector('#unitMultiplier') || document.getElementById('unitMultiplier');
            if (nameInput) nameInput.value = u.name || '';
            if (abbrInput) abbrInput.value = u.abbreviation || '';
            if (multInput) multInput.value = typeof u.multiplier !== 'undefined' ? parseFloat(u.multiplier).toFixed(4) : '1.0000';
        }
    } else {
