...
function initializeAdmin() {
    ...
    if (openFuelPanelBtn) {
        openFuelPanelBtn.addEventListener('click', async () => {
            // switch to the rider fuel tab
            switchTab('rider-fuel');
            try {
                const selId = await loadRidersForFuelSelect();
                const sel = document.getElementById('fuelRiderSelect');
                if (sel) {
                    // if a rider selected in select already, load that; otherwise use auto-selected id
                    const idToLoad = sel.value || selId;
                    if (idToLoad) await loadFuelHistory(idToLoad);
                }
            } catch (e) {
                console.error('Error loading fuel panel data:', e);
            }
        });
    }
...
function switchTab(tabName) {
    ...
    switch(tabName) {
        ...
        case 'riders':
            ...
            break;
        case 'rider-fuel':
            // populate rider select and load history for selected rider
            loadRidersForFuelSelect().then(selId => {
                if (selId) loadFuelHistory(selId).catch(()=>{});
            }).catch(err => { console.error('Error preparing rider fuel tab', err); });
            break;
        case 'order-reports':
...
async function openFuelForRider(riderId) {
    const container = document.getElementById('rider-fuel');
    if (!container) return;
    // switch to the rider fuel tab to activate content
    switchTab('rider-fuel');
