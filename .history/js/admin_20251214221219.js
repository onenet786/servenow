...
async function saveRider() {
    ...
    // Upload rider image and id card if provided
    try {
        const imgFile = document.getElementById('riderImageFile')?.files?.[0] || null;
        if (imgFile) {
            const fd = new FormData();
            fd.append('image', imgFile);
            const resp = await fetch(`${API_BASE}/api/riders/upload-image`, { method: 'POST', headers: { 'Authorization': `Bearer ${authToken}` }, body: fd });
            const j = await resp.json();
            if (j && j.success && j.image_url) riderData.image_url = j.image_url;
        }
        const idFile = document.getElementById('riderIdCardFile')?.files?.[0] || null;
        if (idFile) {
            const fd2 = new FormData();
            fd2.append('image', idFile);
            const resp2 = await fetch(`${API_BASE}/api/riders/upload-image`, { method: 'POST', headers: { 'Authorization': `Bearer ${authToken}` }, body: fd2 });
            const j2 = await resp2.json();
            if (j2 && j2.success && j2.image_url) riderData.id_card_url = j2.image_url;
        }
