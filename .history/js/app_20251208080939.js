        ...
        if (data.success) {
            ...
            data.products.forEach(product => {
                ...
                productCard.querySelectorAll('img').forEach(img => {
                    try {
                        if (img.complete && img.naturalWidth && img.naturalHeight) applyOrientationFit(img);
                    } catch (e) { /* ignore */ }
                });
            });
        }
...
function buildImgTag(src, variants, alt, pid) {
    ...
    if (variants && typeof variants === 'object') {
        ...
        return `<img src="${smallest || src || fallback}" srcset="${entries}" sizes="(max-width: 600px) 50vw, (max-width: 1200px) 33vw, 25vw" alt="${safeAlt}" loading="lazy" decoding="async" onload="applyOrientationFit(this)" onerror="this.onerror=null;this.src='${fallback}'; console.warn('Product image failed to load:', '${pid}', this.src)">`;
    }
    return `<img src="${src || fallback}" alt="${safeAlt}" loading="lazy" decoding="async" onload="applyOrientationFit(this)" onerror="this.onerror=null;this.src='${fallback}'; console.warn('Product image failed to load:', '${pid}', this.src)">`;
}
    ...
    try {
        ...
        const computeAndSet = () => {
            try {
                ...
                // Slightly desaturate and use soft alpha so image still reads well
