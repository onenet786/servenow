...
async function saveRider() {
    ...
    try {
        ...
        if (imgFile) {
            const dataUrl = await readDataUrl(imgFile);
            if (dataUrl) riderData.imageBase64 = String(dataUrl);
        }
        const idFile = document.getElementById('riderIdCardFile')?.files?.[0] || null;
        if (idFile) {
            const idDataUrl = await readDataUrl(idFile);
            if (idDataUrl) riderData.idCardBase64 = String(idDataUrl);
        }
    } catch (e) { console.warn('Rider image conversion failed', e); }
