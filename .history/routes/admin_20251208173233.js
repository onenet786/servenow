...
router.post('/backup-db', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        // Build mysqldump command. Note: including password on the command line may show up in process lists.
        ...
        exec(cmd, { env: process.env }, (err, stdout, stderr) => {
            if (err) {
                console.error('DB backup failed:', err, stderr);
                return res.status(500).json({ success: false, message: 'Backup failed', error: err.message || stderr });
            }
