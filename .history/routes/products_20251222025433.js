...
router.post('/', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        const finalPrice = computePriceFromCost({ paymentTerm: stores[0].payment_term, costPrice: normalizedCost.value });
        if (finalPrice === null) return res.status(400).json({ success: false, message: 'Invalid cost price' });
...
router.put('/:id', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        if (req.body.price !== undefined && cost_price === undefined) {
            return res.status(400).json({ success: false, message: 'Price cannot be updated directly' });
        }
