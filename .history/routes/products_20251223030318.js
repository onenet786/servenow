...
function normalizeSizeVariantsInput(input) {
    try {
        ...
        const byKey = new Map();
        for (const v of raw) {
            const sizeId = v && v.size_id !== undefined ? parseInt(String(v.size_id), 10) : NaN;
            const unitId = v && v.unit_id !== undefined && v.unit_id !== null && String(v.unit_id).trim().length
                ? parseInt(String(v.unit_id), 10)
                : null;
            ...
            const key = `${sizeId}:${unitId === null ? 'null' : unitId}`;
            byKey.set(key, { size_id: sizeId, unit_id: unitId, price: roundMoney(price) });
        }
...
async function ensureProductSizePricesTable(db) {
    try {
        ...
        try { await db.execute('ALTER TABLE product_size_prices ADD UNIQUE KEY uq_product_size_unit (product_id, size_id, unit_id)'); } catch (e) {}
        ...
        if (e && (e.code === 'ER_NO_SUCH_TABLE' || e.code === 'ER_BAD_TABLE_ERROR')) {
            await db.execute(`
                CREATE TABLE IF NOT EXISTS product_size_prices (
                    ...
                    UNIQUE KEY uq_product_size_unit (product_id, size_id, unit_id),
...
async function loadProductSizeVariants(db, productIds) {
    try {
        ...
        const [rows] = await db.execute(
            `
                SELECT psp.product_id, psp.size_id, psp.unit_id, psp.price, psp.sort_order,
    ...
    try {
        ...
        if (hasRequestedVariants) {
            ...
            for (let i = 0; i < requestedVariants.length; i++) {
                ...
                placeholders.push('(?, ?, ?, ?, ?)');
                values.push(result.insertId, v.size_id, v.unit_id ?? null, v.price, i);
            }
            await req.db.execute(
                `INSERT INTO product_size_prices (product_id, size_id, unit_id, price, sort_order) VALUES ${placeholders.join(',')}`,
                values
            );
        }
        ...
        if (variantsProvided) {
            ...
            try {
                ...
                for (let i = 0; i < requestedVariants.length; i++) {
                    const v = requestedVariants[i];
                    placeholders.push('(?, ?, ?, ?, ?)');
                    values.push(id, v.size_id, v.unit_id ?? null, v.price, i);
                }
                await req.db.execute(
                    `INSERT INTO product_size_prices (product_id, size_id, unit_id, price, sort_order) VALUES ${placeholders.join(',')}`,
                    values
