...
router.get('/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const [riders] = await req.db.execute('SELECT * FROM riders WHERE id = ?', [id]);
...
router.post('/', authenticateToken, requireAdmin, [
    ...
    try {
        ...
        const { firstName, lastName, email, phone, password, vehicleType, licenseNumber, fatherName, image_url, id_card_url, idCardNum } = req.body;
        ...
        // Build INSERT dynamically to include optional columns if present
        const baseCols = ['first_name','last_name','email','phone','password','vehicle_type','license_number'];
        const placeholders = Array(baseCols.length).fill('?');
        const values = [firstName, lastName, email, phone, hashedPassword, vehicleType, licenseNumber];
        const extras = [];
        if (await hasColumn(req.db, 'riders', 'father_name')) { baseCols.push('father_name'); placeholders.push('?'); values.push(fatherName || null); extras.push('father_name'); }
        if (await hasColumn(req.db, 'riders', 'image_url')) { baseCols.push('image_url'); placeholders.push('?'); values.push(image_url || null); extras.push('image_url'); }
        if (await hasColumn(req.db, 'riders', 'id_card_url')) { baseCols.push('id_card_url'); placeholders.push('?'); values.push(id_card_url || null); extras.push('id_card_url'); }
        if (await hasColumn(req.db, 'riders', 'id_card_num')) { baseCols.push('id_card_num'); placeholders.push('?'); values.push(idCardNum || null); extras.push('id_card_num'); }
        const insertSql = `INSERT INTO riders (${baseCols.join(',')}) VALUES (${placeholders.join(',')})`;
...
router.put('/:id', authenticateToken, requireAdmin, [
    ...
    try {
        ...
        const { id } = req.params;
        const { firstName, lastName, email, phone, vehicleType, licenseNumber, isAvailable, isActive, fatherName, image_url, id_card_url, idCardNum } = req.body;
        ...
        if (fatherName !== undefined) {
            const has = await hasColumn(req.db, 'riders', 'father_name');
            if (has) { updateFields.push('father_name = ?'); updateValues.push(fatherName); }
        }
        if (image_url !== undefined) {
            const has = await hasColumn(req.db, 'riders', 'image_url');
            if (has) { updateFields.push('image_url = ?'); updateValues.push(image_url); }
        }
        if (id_card_url !== undefined) {
            const has = await hasColumn(req.db, 'riders', 'id_card_url');
            if (has) { updateFields.push('id_card_url = ?'); updateValues.push(id_card_url); }
        }
        if (idCardNum !== undefined) {
            const has = await hasColumn(req.db, 'riders', 'id_card_num');
            if (has) { updateFields.push('id_card_num = ?'); updateValues.push(idCardNum); }
        }
