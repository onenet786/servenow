...
function computePriceFromCost({ paymentTerm, costPrice, adjustmentType, adjustmentValue }) {
    const base = Number(costPrice);
    const adjVal = Number(adjustmentValue);
    if (!Number.isFinite(base) || base < 0) return null;
    if (!Number.isFinite(adjVal) || adjVal < 0) return null;
    const type = String(adjustmentType || '').toLowerCase();
    const adj = type === 'percent' ? (base * (adjVal / 100)) : adjVal;
    const hasDiscount = String(paymentTerm || '').toLowerCase().includes('with discount');
    const out = hasDiscount ? (base - adj) : (base + adj);
...
router.post('/', authenticateToken, requireStoreOwner, [
    ...
    body('pricing_adjustment_type').optional().isIn(['percent', 'amount']).withMessage('Invalid pricing adjustment type'),
    body('pricing_adjustment_value').optional().isFloat({ min: 0 }).withMessage('Pricing adjustment value must be a positive number'),
    ...
    try {
        ...
        const pricing_adjustment_type = req.body.pricing_adjustment_type;
        const pricing_adjustment_value = req.body.pricing_adjustment_value;
        ...
        const computedPrice = (cost_price !== undefined && cost_price !== null && cost_price !== '') ||
            (pricing_adjustment_type !== undefined && pricing_adjustment_type !== null && pricing_adjustment_type !== '') ||
            (pricing_adjustment_value !== undefined && pricing_adjustment_value !== null && pricing_adjustment_value !== '')
            ? computePriceFromCost({
                paymentTerm: stores[0].payment_term,
                costPrice: cost_price,
                adjustmentType: pricing_adjustment_type,
                adjustmentValue: pricing_adjustment_value
            })
            : null;

        const finalPrice = computedPrice !== null ? computedPrice : (() => {
        ...
        if (finalPrice === null) {
            return res.status(400).json({
                success: false,
                message: 'Price is required, or provide cost price + adjustment to calculate it'
            });
        }
        ...
        const normalizedCostPrice = (cost_price !== undefined && cost_price !== null && String(cost_price).trim() !== '')
            ? Number(cost_price)
            : null;

        const insertFields = ['name','description','cost_price','price','image_url','category_id','store_id','stock_quantity'];
        const insertPlaceholders = ['?','?','?','?','?','?','?','?'];
        const insertValues = [name, description, normalizedCostPrice, finalPrice, image_url, category_id, store_id, stock_quantity];
...
router.put('/:id', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        const shouldComputePrice = (cost_price !== undefined) ||
            (pricing_adjustment_type !== undefined) ||
            (pricing_adjustment_value !== undefined);

        if (shouldComputePrice) {
            const computed = computePriceFromCost({
                paymentTerm: product.payment_term,
                costPrice: cost_price !== undefined ? cost_price : product.cost_price,
                adjustmentType: pricing_adjustment_type,
                adjustmentValue: pricing_adjustment_value
            });
            if (computed === null) {
                return res.status(400).json({
                    success: false,
                    message: 'Invalid cost price or adjustment values'
                });
            }
            updateFields.push('price = ?');
            updateValues.push(computed);
