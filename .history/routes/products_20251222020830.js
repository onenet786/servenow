...
function computePriceFromCost({ paymentTerm, costPrice }) {
    const base = Number(costPrice);
    if (!Number.isFinite(base) || base < 0) return null;
    const hasDiscount = String(paymentTerm || '').toLowerCase().includes('with discount');
    const out = hasDiscount ? (base * 0.9) : (base * 1.15);
    const rounded = Math.round(out * 100) / 100;
    return Math.max(0, rounded);
}

function normalizeNullableNumber(val) {
    if (val === undefined) return { present: false, value: null, ok: true };
    if (val === null) return { present: true, value: null, ok: true };
    const s = String(val).trim();
    if (s.length === 0) return { present: true, value: null, ok: true };
    const n = Number(s);
...
router.post('/', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        const normalizedCost = normalizeNullableNumber(cost_price);
        ...
        const computedPrice = normalizedCost.value !== null
            ? computePriceFromCost({ paymentTerm: stores[0].payment_term, costPrice: normalizedCost.value })
            : null;
        ...
        if (finalPrice === null) {
            return res.status(400).json({
                success: false,
                message: 'Provide cost price to calculate price, or provide price directly'
            });
        }
        ...
        const insertValues = [name, description, normalizedCost.value, finalPrice, image_url, category_id, store_id, stock_quantity];
...
router.put('/:id', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        if (cost_price !== undefined) {
            const normalizedCost = normalizeNullableNumber(cost_price);
            if (!normalizedCost.ok) {
                return res.status(400).json({ success: false, message: 'Cost price must be a positive number' });
            }
            updateFields.push('cost_price = ?');
            updateValues.push(normalizedCost.value);
            if (normalizedCost.value !== null) {
                const computed = computePriceFromCost({ paymentTerm: product.payment_term, costPrice: normalizedCost.value });
                if (computed === null) {
