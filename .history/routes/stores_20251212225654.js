...
router.post('/', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        const [result] = await req.db.execute(
            `INSERT INTO stores (name, description, location, latitude, longitude, delivery_time, opening_time, closing_time, phone, email, address, owner_id, cover_image, category_id)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [name, description || null, location, latitude || null, longitude || null, delivery_time || null, opening_time || null, closing_time || null, phone || null, email || null, address || null, ownerId, image_url || null, req.body.category_id || null]
        )
