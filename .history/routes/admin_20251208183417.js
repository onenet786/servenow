...
router.post('/backup-db', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        dump.on('close', (code) => {
            ...
            if (responded) return;
            responded = true;
            // If mysqldump exited with non-zero code, report stderr and remove file
            try {
                if (code !== 0) {
                    console.error('DB backup failed with code:', code, stderrAccum);
                    // remove partial file if exists
                    try { if (fs.existsSync(filepath)) fs.unlinkSync(filepath); } catch(e) { console.error('Failed to remove partial dump:', e); }
                    return res.status(500).json({ success: false, message: 'Backup failed', exitCode: code, error: stderrAccum });
                }

                // If file exists but is empty, treat as failure (mysqldump produced no output)
                const st = fs.existsSync(filepath) ? fs.statSync(filepath) : null;
                if (!st || st.size === 0) {
                    console.error('DB backup produced empty file.', stderrAccum);
                    try { if (fs.existsSync(filepath)) fs.unlinkSync(filepath); } catch(e) { console.error('Failed to remove empty dump:', e); }
                    return res.status(500).json({ success: false, message: 'Backup produced no output', error: stderrAccum });
                }

                return res.json({ success: true, filename, downloadUrl: `/api/admin/backup-db/download?file=${encodeURIComponent(filename)}` });
            } catch (e) {
                console.error('Error finalizing backup:', e);
                try { if (fs.existsSync(filepath)) fs.unlinkSync(filepath); } catch(_){ }
                return res.status(500).json({ success: false, message: 'Backup finalize failed', error: e.message });
            }
        });
    } catch (err) {
