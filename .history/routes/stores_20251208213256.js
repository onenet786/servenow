...
router.post('/', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        // Check if 'cover_image' column exists in the stores table (for compatibility)
        const [cols] = await req.db.execute(`
            SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'stores' AND COLUMN_NAME = 'cover_image'
        `);

        let result;
        if (cols && cols.length > 0) {
            [result] = await req.db.execute(
                `INSERT INTO stores (name, description, location, latitude, longitude, delivery_time, opening_time, closing_time, phone, email, address, cover_image, owner_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [name, description || null, location, latitude || null, longitude || null, delivery_time || null, opening_time || null, closing_time || null, phone || null, email || null, address || null, image_url || null, ownerId]
            );
        } else {
            [result] = await req.db.execute(
...
router.put('/:id', authenticateToken, requireStoreOwner, [
    ...
    try {
        ...
        if (req.user.user_type !== 'admin' && store.owner_id !== req.user.id) {
            ...
            // Check if 'cover_image' column exists in the stores table (for compatibility)
            const [colCheck] = await req.db.execute(`
                SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'stores' AND COLUMN_NAME = 'cover_image'
            `);
            const hasCoverImage = Array.isArray(colCheck) && colCheck.length > 0;
        ...
        if (req.body.image_url !== undefined && hasCoverImage) { updateFields.push('cover_image = ?'); updateValues.push(req.body.image_url); }
