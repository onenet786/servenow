...
function normalizeSizeVariantsInput(input) {
    try {
        ...
        for (const v of raw) {
            ...
            bySizeId.set(sizeId, { size_id: sizeId, price: roundMoney(price) });
        }
...
async function loadProductSizeVariants(db, productIds) {
    try {
        ...
        const [rows] = await db.execute(
            `
                SELECT psp.product_id, psp.size_id, psp.price, psp.sort_order, sz.label as size_label
        ...
        res.json({
            ...
            products: products.map(product => ({
                ...
                size_variants: (variantsByProductId[product.id] && variantsByProductId[product.id].length)
                    ...
                    : (product.size_id ? [{ size_id: product.size_id, size_label: product.size_label || null, price: Number(product.price) }] : [])
            }))
        });
    ...
    try {
        ...
        if (hasRequestedVariants) {
            ...
            for (let i = 0; i < requestedVariants.length; i++) {
                ...
                placeholders.push('(?, ?, ?, ?)');
                values.push(result.insertId, v.size_id, v.price, i);
            }
            await req.db.execute(
                `INSERT INTO product_size_prices (product_id, size_id, price, sort_order) VALUES ${placeholders.join(',')}`,
                values
            );
        }
        ...
        if (variantsProvided) {
            ...
            try {
                ...
                for (let i = 0; i < requestedVariants.length; i++) {
                    const v = requestedVariants[i];
                    placeholders.push('(?, ?, ?, ?)');
                    values.push(id, v.size_id, v.price, i);
                }
                await req.db.execute(
                    `INSERT INTO product_size_prices (product_id, size_id, price, sort_order) VALUES ${placeholders.join(',')}`,
                    values
