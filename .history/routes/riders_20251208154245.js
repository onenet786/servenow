...
router.get('/fuel-history', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const orderColAll = colNamesAll.includes('entry_date') ? 'fh.entry_date' : (colNamesAll.includes('created_at') ? 'fh.created_at' : 'fh.id');

        const [rows] = await req.db.execute(
            `SELECT fh.id, fh.rider_id, fh.entry_date, fh.start_meter, fh.end_meter, fh.distance, fh.petrol_rate, fh.fuel_cost, fh.notes, fh.created_at, fh.updated_at, r.first_name, r.last_name
...
router.get('/:id/fuel-history', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const orderCol = colNames.includes('entry_date') ? 'entry_date' : (colNames.includes('created_at') ? 'created_at' : 'id');

        const [rows] = await req.db.execute(
            `SELECT id, rider_id, entry_date, start_meter, end_meter, distance, petrol_rate, fuel_cost, notes, created_at, updated_at
             FROM riders_fuel_history WHERE rider_id = ? ORDER BY ${orderCol} DESC, id DESC`,
            [id]
        );
...
router.get('/debug/fuel-history/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const dbgOrderCol = colNamesDbg.includes('entry_date') ? 'entry_date' : (colNamesDbg.includes('created_at') ? 'created_at' : 'id');
        const [sample] = await req.db.execute(`SELECT id, rider_id, entry_date, start_meter, end_meter, distance, petrol_rate, fuel_cost, notes, created_at, updated_at FROM riders_fuel_history WHERE rider_id = ? ORDER BY ${dbgOrderCol} DESC, id DESC LIMIT 5`, [id]);
