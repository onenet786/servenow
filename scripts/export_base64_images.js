  ...
  try {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        // If sharp is available, produce resized variants
        let sharpLib = null;
        try { sharpLib = require('sharp'); } catch (e) { /* ignore */ }

        if (sharpLib) {
          const sizes = [320, 640, 1024];
          for (const w of sizes) {
            const vname = `product_${id}_${Date.now()}_${w}${ext}`;
            const vpath = path.join(uploadDir, vname);
            try {
              await sharpLib(filePath).resize({ width: w }).toFile(vpath);
              // optional: could store variants mapping somewhere
            } catch (err) {
              console.warn('Failed to write variant', vname, err.message);
            }
          }
        }
